...
- Im Bestands-Modus spiegeln die Bauteil-Schalter den Bestand; fehlt ein Zusatz, wird er weggelassen statt das Modell zu verkleinern (Reihenfolge: Dach, Kletterwand, Höhlenwände, zuletzt nacktes Gerüst) · In inventory mode the part switches mirror the inventory; a missing extra is dropped instead of shrinking the model (order: roof, climbing wall, cave walls, finally a bare frame)
