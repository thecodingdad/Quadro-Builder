...
### Hinzugefügt · Added
- **Platten liegen auf den Rohren, nicht in ihnen** – neue Eigenschaft `side`: oben auf bzw. außen (Standard) oder darunter bzw. innen. Beim Setzen entscheidet der Blickwinkel, ein Klick auf eine liegende Platte legt sie um · **Panels rest on the tubes instead of inside them** – new `side` property: on top/outside (default) or underneath/inside. The viewing angle decides when placing, a click on a placed panel flips it
